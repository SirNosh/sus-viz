...
function Header({ mode, onModeChange }: { mode: "story" | "explore"; onModeChange: (mode: "story" | "explore") => void }) {
  return (
    <header className="site-header">
      <a className="wordmark" href="#top" aria-label="The Distance Remaining, back to top"><span>JHU</span> / CLIMATE + SUSTAINABILITY</a>
      <nav className="mode-switch" aria-label="View mode">
        <button className={mode === "story" ? "active" : ""} onClick={() => onModeChange("story")}>Story</button>
        <button className={mode === "explore" ? "active" : ""} onClick={() => onModeChange("explore")}>Explore data</button>
      </nav>
    </header>
  );
}
